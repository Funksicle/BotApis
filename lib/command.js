...
class Input {
    constructor(req, res, args) {
        ...
        this.level = parseInt(req.query["level"]);
        if (args)
...
export class Command {
    constructor(commandOptions) {
        ...
        this.security = commandOptions.security;
    ...
    register(app) {
        ...
        logger.info(`Registered the ${logger.colour.bold}${this.command}${logger.colour.reset} command!\n   !editcom ${this.command} fetch[${this.endpoint}]`);
    }
    cb = (req, res) => { // arrow function to get around this/that fuckery
        try {
            ...
            if (this.security && this.security.minLevel > input.level)
                return input.replyTo(`!${this.command} requires a level of ${this.security.minLevel}, you're level ${input.level}. Chat more!`);
            if (input.error)
                return input.replyTo(this.help.message(input.error));
            this.callback(input);
        } catch (err) {
            logger.error(`Error registering ${this.command}`, err);
        }
    }
}
