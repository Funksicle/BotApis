...
function validCurrencyOrDefaults(currencyCodes) {
    ...
    if (!valid.length) // none valid or none provided, show defaults
        return "USD,INR,AUD,GBP,EUR";
...
export class JackieCurrency {
    ...
    async callback(req, res) {
        try {
            ...
            const result = JSON.parse(await webGet(`https://api.freecurrencyapi.com/v1/latest?apikey=${config.get("currencyapi_key")}&currencies=${validCurrencies}&base_currency=${convertFrom.code}`));
