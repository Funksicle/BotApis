import { readFileSync, uservariables } from "./deps.js";
...
export function setTimezone(res, timezone) {
    if (!timezone) {
        const tz = uservariables.get("timezone");
        if (!tz)
            return res.end(`You need to provide a valid IANA timezone code. For example, Thailand's timezone is "Asia/Bangkok", and not simply "Thailand". Others are sometimes just the country name, like Japan. Google it!`);
        return res.end(`The timezone is currently set to ${tz}.`);
    }
    const tz = getValidTimezone(timezone);
    if (tz) {
        uservariables.set("timezone", tz);
        return res.end(`The timezone was set to ${tz}.`);
