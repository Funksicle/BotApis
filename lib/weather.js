...
// for some reason aws doesn't let me use fetch on open-meteo so we'll curl it. blah
import { curl } from "./web.js";
...
export const jackieWeather = new Route("/jackieweather", async function (req, res) {
    try {
        ...
        const url = encodeURI(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${long}&current=temperature_2m,relative_humidity_2m,apparent_temperature,is_day,precipitation,rain,showers,snowfall,cloud_cover,wind_speed_10m,wind_direction_10m&timezone=${tz}&forecast_days=1`);
        const weatherResponse = await curl([ "--compressed", "-sSL", url ]);
