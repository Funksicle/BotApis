import { webcurl, Route } from "./deps.js"; // open-meteo doesn't like fetch on AWS. no idea why, use curl
...
export const jackieWeather = new Route("/jackieweather", async function (req, res) {
    try {
        ...
        const temperature = `In ${location}, it is ${json.current.temperature_2m}${json.current_units.temperature_2m} (${celsiusToFahrenheit(json.current.temperature_2m)}°F), `;
