...
function sanitise(text) {
    let response = text
        ...
        .replace(/  /g, " ")
        .replace(/\[|\]/g, "");
    if (response.length > 199)
        return response.slice(0, 199);
    return response;
}
...
export const jackieUrbanDictionary = new Route("/jackieurbandictionary", async function (req, res) {
    try {
        ...
        const example = result.example.length ? ` - example: ${sanitise(result.example)}` : "";
        const response = `${result.word}: ${sanitise(result.definition)}${example}`;
        res.end(response.replace(/  /g, " "));
    } catch (err) {
        console.error(err);
    }
});